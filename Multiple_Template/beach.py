...
def beach_run():
    ...
    place = remove_random_line('/home/lettuce/WorkCode/ChatGPTWordPRess/beaches.txt')

    print(place)

    #End Links in all
    prompt = f"Generate a travel blog about beaches for {place}"
    title_prompt = f"Generate a travel blog title about beaches for {place}"

    time.sleep(2)
    img_prompt_1 = f"{place} beach"
    ...
    prompt_1 = f"I have this travel article about beaches can you write an adtional section for {prompt} 300 words. This is what I have so far{article_block_content_1}"
    ...
    img_prompt_3 = f"{place} tourist attractions beach"
    prompt_2 = f"I have this travel article about beaches can you write an adtional section for {prompt} 300 words. This is what I have so far{addtion_block_prompt_2}"
    addtion_block_prompt_3 = AIAPI.article_block_generate(prompt_2)
    img3 = imgAPI.get_img(img_prompt_3)

    time.sleep(2)
    prompt_4 = f"Write a conclusion on why to go to this beach at {place} as a travel article"
    ...
    #template if theme
    time.sleep(2)
    r = requests.post(url= ROOT + '/wp-json/wp/v2/beache', data=data, headers=headers, auth=(user, password))
    print(r)
