...
def money_run():
    ...
    place = remove_random_line('/home/lettuce/WorkCode/ChatGPTWordPRess/money.txt')
    print(place)


    #End Links in all
    prompt = f"Generate a blog about traveling to {place} for vacation for cheap on a budget"
    title_prompt = f"Generate a travel blog title about a budget trip for {place}"

    time.sleep(2)
    img_prompt_1 = f"{place}"
    article_block_content_1 = AIAPI.article_block_generate(prompt)
    img1 = imgAPI.get_img(img_prompt_1)

    time.sleep(2)
    img_prompt_2 = f"{place} beautiful"
    prompt_1 = f"I have this travel article about traveling to {place} for cheap vacation on a budget can you write an adtional section for it in 300 words. This is what I have so far{article_block_content_1}"
    ...
    img_prompt_3 = f"{place} tourist attraction"
    prompt_2 = f"I have this travel article about traveling to {place} for cheap vacation on a budget can you write an adtional section for it in 300 words. This is what I have so far{addtion_block_prompt_2}"
    addtion_block_prompt_3 = AIAPI.article_block_generate(prompt_2)
    img3 = imgAPI.get_img(img_prompt_3)

    time.sleep(2)
    prompt_4 = f"Write a conclusion on why to go to this place on a budget vacation at {place} as a travel article"
    ...
    time.sleep(2)  
    try:
        r = requests.post(url= ROOT + '/wp-json/wp/v2/cheap_place', data=data, headers=headers, auth=(user, password))
